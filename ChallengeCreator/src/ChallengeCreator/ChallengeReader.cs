...
internal class ChallengeReader
{
    ...
    public static IEnumerator GetCurrentChallengeRoutine(Action onComplete)
    {
        ...
        if (input.StartsWith("CHALLENGE_") || !input.StartsWith("{"))
        {
            ...
            using (UnityWebRequest www = UnityWebRequest.Get(url))
            {
                ...
                else
                {
                    ...
                    if (response != null && response.Count > 0)
                    {
                        ...
                        Plugin.Log.LogInfo($"Fetched fresh config for: {shortId}");
                    }
                }
            }
        }
    ...
    {
        return JsonConvert.SerializeObject(currentChallenge);
    }
    ...
    public static void LoadChallengeFromJson(string json)
    {
        ...
        {
            var settings = new JsonSerializerSettings { MissingMemberHandling = MissingMemberHandling.Ignore };
            CurrentChallenge decoded = JsonConvert.DeserializeObject<CurrentChallenge>(json, settings);
            if (decoded != null)
            {
                currentChallenge = decoded;
                Plugin.Log.LogInfo($"Synced Challenge: {currentChallenge.Name}");
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogError($"Sync Error: {ex.Message}");
        }
    }
